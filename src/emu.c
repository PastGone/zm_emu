...
#undef ZM_MAP
	...
	log_info("内存映射完成（客户机堆：%s，%u 字节）",
			 g_ulibc_heap ? "ulibc 真实堆" : "bump 分配器（排查用）",
			 HEAP_SIZE);
	return 0;
}
...
int zm_emu_load_blob(FILE *fp, const long *applet_size) {
	...
	if (bytes_read != (size_t)*applet_size) {
		log_error("读取文件失败，期望%zu字节，实际读取%zu", (size_t)*applet_size, bytes_read);
		free(buf);
		fclose(fp);
		return -1;
	}
...
int zm_emu_add_hooks() {
	...
	uc_err err;
	log_info("添加钩子");
	err = uc_hook_add(g_uc, &hook_code_handle, UC_HOOK_CODE, (void *)hook_code, NULL, 1, 0);
	if (err != UC_ERR_OK) {
		log_error("uc_hook_add failed, err: %d\n", err);
		return -1;
	}

	err = uc_hook_add(g_uc,
					  &hook_unmapped_mem_handle,
					  UC_HOOK_MEM_UNMAPPED,
					  (void *)hook_mem_unmapped,
					  NULL,
					  1,
					  0);
	if (err != UC_ERR_OK) {
...
static void trace_code_hook(uc_engine *uc, uint64_t address, uint32_t size, void *user) {
	(void)uc;
	(void)size;
	(void)user;
...
int zm_emu_start_applet() {
	 ...
	 * 用来查"某字段有没有人写、是谁写的"。详见 hook.h。 */
	{
		...
		if (p && p[0]) {
			...
			if (end && *end == ',')
				hi = strtoul(end + 1, NULL, 0);
			uc_hook hw;
			uc_err we = uc_hook_add(g_uc,
									&hw,
									UC_HOOK_MEM_WRITE,
									(void *)hook_mem_write_watch,
									NULL,
									(uint64_t)lo,
									(uint64_t)hi);
	...
	if (e == UC_ERR_OK) {
		...
		static const int regs[] = {UC_ARM_REG_R0,
								   UC_ARM_REG_R1,
								   UC_ARM_REG_R2,
								   UC_ARM_REG_R3,
								   UC_ARM_REG_R4,
		 ...
		 * 那里 ARM 解码是合法的 `str fp,[sp]`。 */
		{
			...
			if (uc_reg_read(g_uc, UC_ARM_REG_CPSR, &cpsr) == UC_ERR_OK)
				log_error("CPSR=0x%X（解释模式：%s）", cpsr, (cpsr & 0x20) ? "Thumb" : "ARM");
		}
