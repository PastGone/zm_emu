...
bool zm_event_async_poll(uc_engine *uc, uint32_t resume_pc, bool starved) {
	...
	uint32_t args[5] = {g_instance, evt, x, (evt == APP_CMD_INIT) ? INIT_CTX : y, g_instance};
...
void dispatch_applet_event(uint32_t evt, uint32_t x, uint32_t y) {
	...
	if (g_disasm)
		log_debug("派发事件 evt=%u (x=%u y=%u) -> handler=0x%X", evt, x, y, g_handler);
	...
	if (g_disasm) {
		...
		log_debug("payload[0x180]=0x%X  *(0x180)=0x%X  *(*0x180)=0x%X", BLOB_BASE, slot, fn);
	}
...
void on_touch_click(uint32_t x, uint32_t y) {
	log_info("触摸事件: (%u, %u) -> handler=0x%X instance=0x%X", x, y, g_handler, g_instance);
...
bool zm_event_dispatch_pending(void) {
	if (!g_pending_touch.has_pending)
		return false;
	dispatch_applet_event(g_pending_touch.evt, g_pending_touch.x, g_pending_touch.y);
	g_pending_touch.has_pending = false;
	return true;
}
...
static const char *evt_name(uint32_t cmd) {
	...
	case APP_CMD_RESUME:
		return "RESUME";
	case APP_CMD_REPAINT:
		return "REPAINT";
	case APP_CMD_KEY_DOWN:
		return "KEY_DOWN";
	case APP_CMD_KEY_UP:
		return "KEY_UP";
	case APP_CMD_KEY_LONG_PRESS:
		return "KEY_LONG_PRESS";
	case APP_CMD_KEY_MULTIPLE:
		return "KEY_MULTIPLE";
	case APP_CMD_TOUCH_DOWN:
		return "TOUCH_DOWN";
	case APP_CMD_TOUCH_UP:
		return "TOUCH_UP";
	case APP_CMD_TOUCH_MOVE:
		return "TOUCH_MOVE";
	default:
		return "?";
	}
...
void zm_event_send(uint32_t cmd, uint32_t arg1, uint32_t arg2) {
	...
	if (cmd >= APP_CMD_KEY_DOWN && cmd <= APP_CMD_KEY_MULTIPLE)
		log_info("派发 %s(%u) a1=%u -> handler=0x%X instance=0x%X",
				 evt_name(cmd),
				 cmd,
				 arg1,
				 g_handler,
		...
		log_debug(
			"派发 %s(%u) a1=%u a2=%u -> handler=0x%X", evt_name(cmd), cmd, arg1, arg2, g_handler);
...
void zm_event_repaint(void) {
	zm_event_send(APP_CMD_REPAINT, 0, 0);
}
...
bool zm_event_key_from_sdl(int sdl_sym, uint32_t *out_code) {
	static const struct {
		...
		{SDLK_w, KEYCODE_DPAD_UP},
		{SDLK_s, KEYCODE_DPAD_DOWN},
		{SDLK_a, KEYCODE_DPAD_LEFT},
		{SDLK_d, KEYCODE_DPAD_RIGHT},
		...
		{SDLK_q, KEYCODE_SOFT_LEFT},
		{SDLK_e, KEYCODE_SOFT_RIGHT},
		{SDLK_ESCAPE, KEYCODE_BACK},
		{SDLK_BACKSPACE, KEYCODE_BACK},
		{SDLK_AC_BACK, KEYCODE_BACK},
		/* 拨号 Z / 星号 N / 井号 M（PC 上的 * 与 # 键也一并映射） */
		{SDLK_z, KEYCODE_CALL},
		{SDLK_n, KEYCODE_STAR},
		{SDLK_ASTERISK, KEYCODE_STAR},
		{SDLK_KP_MULTIPLY, KEYCODE_STAR},
		{SDLK_m, KEYCODE_POUND},
		{SDLK_HASH, KEYCODE_POUND},
		/* 中心/确认：空格 / 回车 */
		{SDLK_SPACE, KEYCODE_CENTER},
		{SDLK_RETURN, KEYCODE_CENTER},
		{SDLK_KP_ENTER, KEYCODE_CENTER},
		/* Home H / Search F */
		{SDLK_h, KEYCODE_HOME},
		{SDLK_f, KEYCODE_SEARCH},
