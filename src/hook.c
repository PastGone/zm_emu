...
void hook_set_pc_watch_idx(int idx, uint32_t lo, uint32_t hi) {
	...
	g_pc_watch_on = 1;
	log_info("[PC观察%d] 命中区间 0x%X ~ 0x%X 时打印 PC/LR/R0-R3", idx + 1, lo, hi);
}
...
void hook_dump_pc_ring(void) {
	...
	for (int k = 0; k < shown; k++) {
		...
		char hex[3 * 8 + 1];
		int q = 0;
		if (s_pc_uc && uc_mem_read(s_pc_uc, s_pc_ring[i], b, sizeof(b)) == UC_ERR_OK)
...
void hook_code(uc_engine *uc, uint64_t address, uint32_t size, void *user_data) {
	...
	if (g_pc_watch_on) {
		for (int i = 0; i < PC_WATCH_MAX; i++) {
			if (!g_pc_watch_hi[i] || address < g_pc_watch_lo[i] || address > g_pc_watch_hi[i] ||
				g_pc_watch_n[i] >= 400)
				continue;
			uint32_t r[8] = {0}, lr = 0;
...
void hook_shim_mem(
	...
	if (type == UC_MEM_READ) {
		...
		s_shim_r_n++;
		log_debug(
			"[HOOK] 读 0x%X (%d 字节) = 0x%" PRIx64, (uint32_t)address, size, (uint64_t)value);
		...
		s_shim_w_n++;
		log_debug(
			"[HOOK] 写 0x%X (%d 字节) = 0x%" PRIx64, (uint32_t)address, size, (uint64_t)value);
	}
}
...
bool hook_mem_unmapped(
	...
	log_warn("  !! MEM unmapped @0x%" PRIx64 " size=%d"
			 ...
			 "  [0x180]=0x%X [ROOT_TABLE_ADDR]=0x%X [ROOT_TABLE_ADDR+8]=0x%X "
			 "[ROOT_TABLE_ADDR+C]=0x%X\n",
			 address,
			 size,
			 pc,
			 lr,
...
uint32_t hook_ctx_candidate(void) {
	return g_ctx_cand;
}
