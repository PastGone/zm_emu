...
#define ZM_SCREEN_PRESET_N ((int)(sizeof(k_screen_presets) / sizeof(k_screen_presets[0])))
...
static const char *zm_arg_val(int argc, char **argv, int *i, const char *eqval) {
	if (eqval) {
		return eqval;
	}
	if (*i + 1 < argc) {
...
int main(int argc, char **argv) {
	...
	char *filename =
		"applet/00000102/"
		"00000102.app"; // 其他的用外部框架测试脚本来测试默认，保留这个是因为这个最简单目前如果连这个都跑不了那其他的也更别想跑
	 ...
	 */
	{
		...
		if (sel && *sel) {
			...
			if (strchr(sel, '/') || strchr(sel, '\\'))
				...
				snprintf(applet_path, sizeof(applet_path), "applet/%s/%s.app", sel, sel);
	 ...
	 *   4) 最后再按编译期容量上限 LAYER_MAX_W/H 夹紧（静态数组按上限预留）。 */
	{
		...
		if (scr && *scr) {
			...
			if (sscanf(scr, "%dx%d", &ew, &eh) == 2 && ew > 0 && eh > 0) {
				...
				h = eh;
				src = "ZM_SCREEN=宽x高";
			} else if (end && end != scr && *end == '\0' && n >= 0 && n < ZM_SCREEN_PRESET_N) {
				w = k_screen_presets[n].w;
				h = k_screen_presets[n].h;
				src = "ZM_SCREEN=序号";
			} else {
				log_warn("ZM_SCREEN 无法识别: %s（可用 宽x高，如 320x480；或预设序号）", scr);
			}
		}
		...
		if (minW > 0 && maxW >= minW && (w < minW || w > maxW)) {
			...
			log_warn("屏宽 %d 超出该 applet 支持区间 %d~%d，夹到 %d", w, minW, maxW, cw);
			w = cw;
		}
		if (minH > 0 && maxH >= minH && (h < minH || h > maxH)) {
			int ch = (h < minH) ? minH : maxH;
			log_warn("屏高 %d 超出该 applet 支持区间 %d~%d，夹到 %d", h, minH, maxH, ch);
			h = ch;
		}
