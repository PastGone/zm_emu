...
static void zm_stat_bump(zm_stat_ent_t *tab, uint32_t *used, uint32_t *total,
                         uint32_t key) {
...
void zm_stat_touch(uint32_t x, uint32_t y) {
  if (!s_on)
    return;
  zm_stat_bump(s_touch, &s_touch_used, &s_touch_total,
               ((y & 0xFFFFu) << 16) | (x & 0xFFFFu));

  /* 打印上一次点击期间的外部调用清单，然后开一个新桶 */
  if (s_click_open) {
    log_info("===== 第 %d 次点击 (%u,%u) 期间触发的外部调用：共 %u 次 =====",
             s_click_seq, s_last_x, s_last_y, s_click_total);
    for (uint32_t i = 0; i < ZM_STAT_CAP; i++) {
      if (s_click[i].cnt) {
        uint32_t slot = 0;
  ...
  s_last_x = x;
  s_last_y = y;
  log_info("===== 第 %d 次点击 (%u,%u)：开始记录此后触发的外部调用 =====",
           s_click_seq, x, y);
}
...
static void zm_stat_dump_table(const char *title, zm_stat_ent_t *tab,
                               uint32_t used, uint32_t total, int is_touch) {
  if (!s_on)
    return;
  ...
  for (int k = 0; k < 20; k++) {
    ...
    if (is_touch) {
      ...
      log_info("  #%2d  点击 (%u,%u)  ×%u", k + 1, key & 0xFFFFu, key >> 16,
               bestc);
    } else {
      uint32_t slot = 0;
