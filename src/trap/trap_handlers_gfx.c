...
uint32_t a_zm_display_AddRef(trap_ctx *c) { return zm_display_AddRef(c->uc, c->r0); }

uint32_t a_zm_display_Release(trap_ctx *c) { return zm_display_Release(c->uc, c->r0); }
...
uint32_t a_zm_display_GetFontHeight(trap_ctx *c) { return zm_display_GetFontHeight(c->uc); }
...
uint32_t a_zm_display_Refresh(trap_ctx *c) { return zm_display_Refresh(c->uc); }
...
uint32_t a_zm_surf_release(trap_ctx *c) { return zm_surf_release(c->uc, c->r0); }

uint32_t a_zm_surf_getrect(trap_ctx *c) { return zm_surf_getrect(c->uc, c->r0, c->r1); }

/* 区间：offset 由 trap 地址自动算 */
uint32_t d_surf_nop(trap_ctx *c) { return zm_surf_nop(c->uc, c->trap - TRAMP_BASE); }

/* ==========================================================================
 * IImage
 * ========================================================================== */

uint32_t a_zm_image_AddRef(trap_ctx *c) { return zm_image_AddRef(c->uc, c->r0); }

uint32_t a_zm_image_Release(trap_ctx *c) { return zm_image_Release(c->uc, c->r0); }

uint32_t a_zm_image_GetFrameCount(trap_ctx *c) { return zm_image_GetFrameCount(c->uc, c->r0); }

uint32_t a_zm_image_Width(trap_ctx *c) { return zm_image_Width(c->uc, c->r0); }

uint32_t a_zm_image_Height(trap_ctx *c) { return zm_image_Height(c->uc, c->r0); }

uint32_t a_zm_image_GetType(trap_ctx *c) { return zm_image_GetType(c->uc, c->r0); }
...
uint32_t a_zm_bitmap_AddRef(trap_ctx *c) { return zm_bitmap_AddRef(c->uc, c->r0); }

uint32_t a_zm_bitmap_Release(trap_ctx *c) { return zm_bitmap_Release(c->uc, c->r0); }

uint32_t a_zm_bitmap_SetTransColor(trap_ctx *c) {
  return zm_bitmap_SetTransColor(c->uc, c->r0, c->r1);
}

uint32_t a_zm_bitmap_GetInfo(trap_ctx *c) { return zm_bitmap_GetInfo(c->uc, c->r0, c->r1); }
