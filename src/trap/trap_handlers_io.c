...
uint32_t a_zm_file_close(trap_ctx *c) { return zm_file_close(c->uc, c->r0); }

uint32_t a_zm_file_read(trap_ctx *c) { return zm_file_read(c->uc, c->r0, c->r1, c->r2); }

uint32_t a_zm_file_write(trap_ctx *c) { return zm_file_write(c->uc, c->r0, c->r1, c->r2); }

uint32_t a_zm_file_seek(trap_ctx *c) { return zm_file_seek(c->uc, c->r0, c->r1, c->r2); }

uint32_t a_zm_file_tell(trap_ctx *c) { return zm_file_tell(c->uc, c->r0); }
...
uint32_t a_cbk_open_file(trap_ctx *c) { return zm_cbk_file_open(c->uc, c->r1, c->r2); }

uint32_t a_cbk_file_release(trap_ctx *c) { return zm_cbk_file_release(c->uc, c->r0); }
...
uint32_t a_cbk_file_size(trap_ctx *c) { return zm_cbk_file_size(c->uc, c->r0); }
