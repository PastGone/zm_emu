...
uint32_t a_enter_event_loop(trap_ctx *c) {
	...
	if (stage == 0) {
		...
		log_info("init 握手完成：size=%u handler=0x%X（API_SLOT=[%08X %08X %08X "
				 ...
				 size,
				 handler,
				 uc_read32(uc, API_SLOT),
				 uc_read32(uc, API_SLOT + 4),
				 uc_read32(uc, API_SLOT + 8),
		...
		if (size == 0 || size > 0x40000) {
			constexpr uint32_t FALLBACK_SIZE = 0x2000;
			log_warn(
				"握手 size 异常（0x%X），按宽松握手使用兜底大小 %u 字节继续", size, FALLBACK_SIZE);
			size = FALLBACK_SIZE;
		}
...
uint32_t a_zm_strcmp(trap_ctx *c) {
	return zm_strcmp(c->uc, c->r0, c->r1);
}

uint32_t a_zm_strstr(trap_ctx *c) {
	return zm_strstr(c->uc, c->r0, c->r1);
}

uint32_t a_zm_strchr(trap_ctx *c) {
	return zm_strchr(c->uc, c->r0, c->r1);
}

uint32_t a_zm_strlen(trap_ctx *c) {
	return zm_strlen(c->uc, c->r0);
}

uint32_t a_zm_wcslen(trap_ctx *c) {
	return zm_wcslen(c->uc, c->r0);
}

uint32_t a_zm_spec_lookup(trap_ctx *c) {
	return zm_spec_lookup(c->uc, c->r0, c->r1);
}
...
uint32_t a_u_memcmp(trap_ctx *c) {
	return u_memcmp(c->uc, c->r0, c->r1, c->r2);
}

uint32_t a_u_memcpy(trap_ctx *c) {
	return u_memcpy(c->uc, c->r0, c->r1, c->r2);
}

uint32_t a_u_memset(trap_ctx *c) {
	return u_memset(c->uc, c->r0, c->r1, c->r2);
}
...
uint32_t a_root_sprintf(trap_ctx *c) {
	...
	if (g_disasm) {
		...
		log_debug("sprintf lr=0x%X [%u参数]: fmt=\"%s\" out=\"%s\" ret=%u",
				  lr,
				  uc_read32(c->uc, c->r2),
				  fmt,
				  outp,
...
uint32_t a_root_sqrt(trap_ctx *c) {
	return zm_root_math(c->uc, ZM_MATH_SQRT, c->r0, c->r1);
}

uint32_t a_root_cos(trap_ctx *c) {
	return zm_root_math(c->uc, ZM_MATH_COS, c->r0, c->r1);
}

uint32_t a_root_sin(trap_ctx *c) {
	return zm_root_math(c->uc, ZM_MATH_SIN, c->r0, c->r1);
}

uint32_t a_root_atan(trap_ctx *c) {
	return zm_root_math(c->uc, ZM_MATH_ATAN, c->r0, c->r1);
}

uint32_t a_root_tan(trap_ctx *c) {
	return zm_root_math(c->uc, ZM_MATH_TAN, c->r0, c->r1);
}
