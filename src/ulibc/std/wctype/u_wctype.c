...
unsigned long u_wctype(const char *name) {
  ...
  static const struct {
    const char *n;
    unsigned long v;
  } tab[] = {{"alnum", WCT_ALNUM}, {"alpha", WCT_ALPHA},
             {"cntrl", WCT_CNTRL}, {"digit", WCT_DIGIT},
             {"graph", WCT_GRAPH}, {"lower", WCT_LOWER},
             {"print", WCT_PRINT}, {"punct", WCT_PUNCT},
             {"space", WCT_SPACE}, {"upper", WCT_UPPER},
             {"xdigit", WCT_XDIGIT}};
