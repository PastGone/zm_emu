...
static void queue_completion_cb(uint32_t cb, uint32_t ctx, uint32_t a1,
  ...
  e->a1 = a1;
  e->a2 = a2;
  log_info("IMedia: 排队完成回调 cb=0x%X(ctx=0x%X, 0x%X, 0x%X)", cb, ctx, a1,
           a2);
}
...
bool zm_media_pending_cb_poll(uc_engine *uc) {
  ...
  log_info("IMedia 完成回调 -> cb=0x%08X(ctx=0x%08X, 0x%08X, 0x%08X)", e.cb,
           e.ctx, e.a1, e.a2);
  return true;
}
...
int zm_audio_init(void) {
   ...
   * 要整体静音（不看 applet 设置）设 ZM_SOUND=0。 */
  {
    ...
    if (s && *s) {
      char c = s[0];
      force_off = (c == '0' || c == 'n' || c == 'N' || c == 'o' || c == 'O' ||
                   c == 'f' || c == 'F'); /* 0/no/off/false */
    }
...
uint32_t zm_media_command(uc_engine *uc, uint32_t r0, uint32_t r1,
  ...
  if (cmd == 0x10 || cmd == 0x40 || cmd == 0x41) {
    ...
    if (n && looks_like_filename((const uint8_t *)name, n)) {
      log_info("IMedia cmd 0x%X: 播放文件 \"%s\"%s", cmd, name,
               loop ? "（循环）" : "");
      uint8_t *fdata = NULL;
      size_t flen = 0;
...
uint32_t zm_media_pause_music(uc_engine *uc) {
  ...
  if (g_sound_force) {
    log_info("IMedia.pauseMusic() lr=0x%X: ZM_SOUND_FORCE 生效 → 忽略本次暂停",
             lr);
    return 0;
  }
