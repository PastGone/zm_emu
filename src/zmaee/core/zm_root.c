	 ...
	 * （`ldrne r1,[r0,#0x10]`，r0=0x7461642E=".dat"）。 */
	{
		...
		if (uc_mem_read(uc, str_obj, &w0, 4) == UC_ERR_OK &&
			...
			if (getenv("ZM_STRCAT_DBG"))
				log_info("[STRCAT] dst=0x%X src=\"%s\" 首两字=0x%X/0x%X inline=0x%X",
						 str_obj,
						 cstr,
						 w0,
						 w1,
		...
		if (ro_data && getenv("ZM_STRCAT_DBG"))
			log_info(
				"[STRCAT] data_ptr 0x%X 在 payload 区 → 改为搬进内联缓冲 0x%X", w0, inline_buf);
		if (!as_obj) {
			...
			if (cur_len + clen > ZM_STRCAT_MAX) {
				log_warn("strcat: 0x%X 追加后超 %d 字节，截断（现有 %u + 追加 %u）",
						 str_obj,
						 ZM_STRCAT_MAX,
						 cur_len,
						 clen);
...
uint32_t zm_root_x68C(uc_engine *uc, uint32_t obj, uint32_t out_buf, uint32_t len, uint32_t self) {
	...
	log_debug("root[0x68C]: obj=0x%X head=%u out=0x%X len=%u", obj, head, out_buf, len);
	return head;
}
