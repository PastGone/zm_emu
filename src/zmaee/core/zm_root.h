#ifndef __ZM_ROOT_H__
#define __ZM_ROOT_H__

// -------------------- ROOT vtable 缺失 trap 的处理函数声明 --------------------
// 00000405.app 通过 [BLOB_BASE+0x180] 取 ROOT 指针后，(*ROOT+offset) 调用大量
// 导入。本模块提供其中缺失槽位的实现：memset / str_assign 给真实实现，
// 其余为 stub（log + 返回 0），便于按日志频次逐个细化为真实实现。
#include <stdint.h>
#include <unicorn/unicorn.h>

/* ROOT[0x60]：memset(dst, val, len) —— sub_841D4 用 MOV R2,#0x214;MOV R1,#0 调用 */
