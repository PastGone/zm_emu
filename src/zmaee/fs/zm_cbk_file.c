...
static cbk_file_t *file_of(uc_engine *uc, uint32_t obj) {
	(void)uc;
	if (obj < CBK_FILE_OBJ || obj >= CBK_FILE_OBJ + CBK_FILE_MAX * CBK_FILE_OBJ_STRIDE)
		return NULL;
...
uint32_t zm_cbk_file_open(uc_engine *uc, uint32_t path_ptr, uint32_t mode) {
	...
	if (!name[0]) {
		log_warn("CBK 文件对象：路径为空（r1=0x%X mode=%u），返回 0", path_ptr, (unsigned)mode);
		return 0;
	}
	...
	log_info(
		"CBK 文件对象：\"%s\" 打开成功（%u 字节），对象=0x%X（槽%d）", name, f->size, obj, slot);
	return obj;
}
...
uint32_t zm_cbk_file_release(uc_engine *uc, uint32_t obj) {
	...
	if (f->dirty && f->data && f->wname[0] && !getenv("ZM_CBK_NO_WRITEBACK")) {
		if (zm_fs_write_back_name(f->wname, f->data, f->size) == 0)
			log_info("CBK 文件对象：\"%s\" 有改动 → **已写回宿主**（%u 字节）", f->wname, f->size);
		else
			log_warn("CBK 文件对象：\"%s\" 写回失败（改动只在内存里 ✗）", f->wname);
	} else if (f->dirty) {
		log_info("CBK 文件对象：\"%s\" 有改动但未落盘（ZM_CBK_NO_WRITEBACK）", f->wname);
	}
	free(f->data);
	f->data = NULL;
	f->size = 0;
