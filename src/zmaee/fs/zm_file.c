	...
	if (file_id == FILE1 && g_file_data) {
		...
		if (g_file_dirty && g_file_path[0]) {
			if (zm_fs_write_back(g_file_path, g_file_data, g_file_size) == 0)
				log_info("file.close: 已写回宿主机 \"%s\"（%zu 字节）", g_file_path, g_file_size);
			else
...
uint32_t zm_file_read(uc_engine *uc, uint32_t file_id, uint32_t buf, uint32_t length) {
	 ...
	 * 前 20 次逐条打印，之后每 500 次汇总一次（避免刷屏）。 */
	{
		...
		if (n == 0)
			nfail++;
		if (nread <= 20)
			log_info("file.read(#%u) buf=0x%X len=%u -> %u (pos=%u size=%zu)",
					 nread,
					 buf,
					 length,
					 n,
			...
			log_info("file.read 累计 %u 次, 累计取回 %u 字节, 空读 %u 次", nread, total, nfail);
	}
	return n;
}
...
uint32_t zm_file_write(uc_engine *uc, uint32_t file_id, uint32_t buf, uint32_t length) {
	...
	if (length <= 64) {
		...
		for (uint32_t i = 0; i < length && n < (int)sizeof(hex) - 4; i++)
			n += snprintf(
				hex + n, sizeof(hex) - (size_t)n, "%02X ", g_file_data[g_file_pos - length + i]);
