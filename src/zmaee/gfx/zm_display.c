...
static TTF_Font *get_font(int font_size) {
	...
	if (!g_font) {
		log_error("TTF_OpenFontIndex(%s, face=%d) failed: %s", fpath, face, TTF_GetError());
		return NULL;
	}
...
static size_t
ucs2_to_utf8(uc_engine *uc, uint32_t ptr, uint32_t max_chars, char *out, size_t cap) {
	size_t o = 0;
...
static inline void fb_px(int x, int y, uint32_t argb) {
	...
	if (g_uc && g_draw_buf && (unsigned)x < (unsigned)g_draw_w &&
		...
		uc_mem_write(
			g_uc, g_draw_buf + ((uint32_t)y * g_draw_w + (uint32_t)x) * 2u, &c, sizeof(c));
	}
}

static inline uint32_t fb_get(int x, int y) {
	if (!g_fb || (unsigned)x >= (unsigned)g_fb_w || (unsigned)y >= (unsigned)g_fb_h)
		return 0;
...
void zm_fb_write(int x, int y, uint32_t argb) {
	fb_px(x, y, argb);
}
...
static size_t text_to_utf8(uc_engine *uc, uint32_t ptr, uint32_t len, char *out, size_t cap) {
	if (cap == 0)
		return 0;
	out[0] = '\0';
	if (!ptr || !len)
...
static void fb_draw_text(uc_engine *uc,
						 uint32_t rect_ptr,
						 const char *text,
						 uint32_t color,
						 int font_size,
 ...
 */
static void fb_blit_rgba_mode(int x, int y, int w, int h, const uint8_t *rgba, int mode) {
		...
		for (int sx = 0; sx <= w; sx++) { /* 多跑一格用于收尾 flush */
			if (sx == w) {
				if (run_n > 0 && g_uc && g_draw_buf)
					uc_mem_write(g_uc,
								 g_draw_buf + ((uint32_t)run_y * g_draw_w + (uint32_t)run_x) * 2u,
								 row16,
								 (size_t)run_n * 2u);
				break;
			}
			...
			if (!row16 || !g_uc || !g_draw_buf || (unsigned)ox >= (unsigned)g_draw_w ||
...
static void fb_blit_rgba_scaled_mode(int x,
									 int y,
									 int dw,
									 int dh,
									 const uint8_t *src,
...
static int save_argb_png(const char *path, const uint32_t *argb, int w, int h) {
	...
	if (!surf) {
		log_warn("截图失败：SDL_CreateRGBSurfaceWithFormatFrom: %s", SDL_GetError());
		return -1;
	}
...
static void fb_present(void) {
	...
	if (prefix && prefix[0] && shot < smax && (present_no % (uint32_t)every) == 0) {
		...
		fb_save_png(path);
		log_info("present #%u (第%d张, 每%d帧): 非黑像素 %d / %d",
				 present_no,
				 shot,
				 every,
				 nonblack,
...
static void zm_seq_dump(void) {
	...
	for (int k = 0; k < ZM_SEQ_N; k++) {
		...
		const char *nm = e->kind == 1	? "SetActiveLayer"
						 ...
						 : e->kind == 4 ? "StretchBlt"
										: "?";
		log_info("   %-15s a0=%-6u a1=0x%-8X act=%u lr=0x%X", nm, e->a0, e->a1, e->act, e->lr);
	}
}
 ...
 * list/count 为空表示不合成。 */
static void
fb_composite(uc_engine *uc, int rx, int ry, int rw, int rh, const uint32_t *list, uint32_t count) {
	if (!uc || !list || count == 0)
		return;
	...
	for (uint32_t step = 0; step < count; step++) {
		...
		if (bpp != 2 && bpp != 4) {
			...
			if (warned < 4) {
				log_warn("[合成] 层%u 的 ZMCF=%u（%d 字节/像素）暂不支持，跳过", li, L.fmt, bpp);
				warned++;
			}
			continue;
		}
		...
		for (int i = 0; i < lh; i++) {
			uint32_t rowoff = L.buf + (uint32_t)(soff_y + i) * (uint32_t)pitch * (uint32_t)bpp +
			...
			uint32_t fb_off = (uint32_t)(y0 + i) * (uint32_t)fb_w * 2u + (uint32_t)x0 * 2u;
			if (use_tc) {
				/* mask：透明像素不动 → 保留下层 / 上一帧的内容（RE：Mask16To16） */
				if (uc_mem_read(uc, FRAMEBUF + fb_off, frow, (size_t)lw * 2u) != UC_ERR_OK)
					break;
				for (int x = 0; x < lw; x++)
					if (row[x] != tc)
						frow[x] = row[x];
...
static void fb_composite_default(void) {
	if (!g_uc)
		return;
	if (g_comp_valid)
		fb_composite(g_uc, g_comp_x, g_comp_y, g_comp_w, g_comp_h, g_comp_list, g_comp_count);
	else
		fb_composite(g_uc, 0, 0, (int)LAYER_W, (int)LAYER_H, ZM_UPDATE_DEFAULT_LIST, 4);
}
...
static int fb_merge_layer(void) {
	...
	int painted = 0;
	for (int y = 0; y < h; y++) {
		if (uc_mem_read(g_uc, FRAMEBUF + (uint32_t)y * LAYER_W * 2u, row, (size_t)w * 2u) !=
			UC_ERR_OK)
			break;
		...
		for (int x = 0; x < w; x++) {
			...
			if (probe_on) {
				...
				if (((c >> 11) & 0x1F) > 0x10 && ((c >> 5) & 0x3F) < 0x08 && (c & 0x1F) > 0x10) {
					nmag++;
					mhist[c]++;
				}
			}
	...
	if (probe_on && (frames % 60) == 0) {
		...
		for (int y = 0; y < h; y++) {
			for (int x = 0; x < w; x++) {
				...
				int r = (int)((c >> 16) & 0xFF), gg = (int)((c >> 8) & 0xFF), b = (int)(c & 0xFF);
				if (r < 24 && gg < 24 && b < 24) {
					nblack++;
					if (x < bx0)
						bx0 = x;
		...
		log_info("  [探针]近黑像素 %d bbox=[%d,%d]-[%d,%d]  暗灰 %d",
				 nblack,
				 bx0,
				 by0,
				 bx1,
		 ...
		 * 有 → applet 确实把背景画在基础层；空 → 它没用这个缓冲。 */
		{
			...
			if (ba) {
				...
				for (int y = 0; y < LAYER_H; y++) {
					if (uc_mem_read(
							g_uc, ba + (uint32_t)y * LAYER_W * 2u, brow, (uint32_t)LAYER_W * 2u) !=
						UC_ERR_OK)
						break;
				...
				log_info("  [探针]基础层 0x%X: 非零 %u/%u (%.1f%%)",
						 ba,
						 nz,
						 total,
		...
		/* 列出所有存在的层：判断"背景是不是在另一层里"（若层 0 存在＝背景层） */
		{
			...
			if (once) {
				...
				log_info("  [探针]共 %d 个层存在；活动层=%u", exist, uc_read32(g_uc, DISPLAY + 8));
			}
		}
		 ...
		 *   层 0 若几乎全透明 → applet 清了它却没画进去（绘制被 stub 丢了） */
		{
			...
			for (uint32_t li = 0; li < ZM_LAYER_COMPOSITE_MAX; li++) {
				...
				for (uint32_t yy = 0; yy < L3.h; yy++) {
					if (uc_mem_read(g_uc, L3.buf + yy * L3.w * 2u, lrow2, (size_t)L3.w * 2u) !=
						UC_ERR_OK)
						break;
...
static void sdlclick_tick(void) {
	...
	e.button.y = wy;
	SDL_PushEvent(&e);
	log_info("ZM_SDLCLICK_T: 到点 %ums → 投递 客户机(%u,%u) 窗口(%d,%d)", at, gx, gy, wx, wy);
}
...
static void sdlkey_tick(void) {
	...
	for (int i = head; i < n; i++) {
		...
		log_info("ZM_SDLKEY_T: %ums → 按下 sym=0x%X%s",
				 q[i].at,
				 (unsigned)q[i].sym,
				 q[i].hold ? "（按住不放）" : "");
	}
...
void zm_display_pump_events(void) {
	...
	while (SDL_PeepEvents(&e, 1, SDL_GETEVENT, SDL_MOUSEBUTTONDOWN, SDL_MOUSEBUTTONDOWN) > 0) {
		int win_w = g_w, win_h = g_h;
		if (g_win)
			SDL_GetWindowSize(g_win, &win_w, &win_h);
		uint32_t cx = (win_w > 0) ? (uint32_t)(e.button.x * g_w / win_w) : (uint32_t)e.button.x;
		uint32_t cy = (win_h > 0) ? (uint32_t)(e.button.y * g_h / win_h) : (uint32_t)e.button.y;
	...
	while (SDL_PeepEvents(&e, 1, SDL_GETEVENT, SDL_KEYDOWN, SDL_KEYUP) > 0) {
		...
		if (zm_event_key_from_sdl((int)e.key.keysym.sym, &code))
			zm_event_queue((e.type == SDL_KEYDOWN) ? APP_CMD_KEY_DOWN : APP_CMD_KEY_UP, code, 0);
	}
	/* ★ 鼠标**移动**事件不能积压。
	 *
	...
	if (SDL_PeepEvents(NULL, 0, SDL_PEEKEVENT, SDL_FIRSTEVENT, SDL_LASTEVENT) > 256) {
		...
		if (dropped > 0 && getenv("ZM_EVQ_DBG"))
			log_info("[事件队列] 丢弃积压的鼠标移动 %d 条（applet 未进输入循环）", dropped);
	}
...
bool zm_display_driver_is_headless(void) {
	...
	return !strcmp(drv, "dummy") || !strcmp(drv, "offscreen") || !strcmp(drv, "null");
}
...
int zm_display_init(void) {
	...
	snprintf(window_title, sizeof(window_title), "%s%s", g_header.AppName, ext_char);
	...
	g_ren = SDL_CreateRenderer(g_win, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_TARGETTEXTURE);
	if (!g_ren) {
		log_warn("SDL_CreateRenderer(accel) failed: %s, 尝试软件渲染", SDL_GetError());
		g_ren = SDL_CreateRenderer(g_win, -1, SDL_RENDERER_SOFTWARE | SDL_RENDERER_TARGETTEXTURE);
	}
	if (!g_ren) {
		log_warn("SDL_CreateRenderer(software+target) failed: %s, 尝试纯软件", SDL_GetError());
	...
	g_canvas =
		SDL_CreateTexture(g_ren, SDL_PIXELFORMAT_ARGB8888, SDL_TEXTUREACCESS_STREAMING, g_w, g_h);
	if (!g_canvas) {
	...
	log_info("zm_display_init: 窗口 %dx%d 已创建（软件帧缓冲 %d KB）",
			 g_w,
			 g_h,
	 ...
	 * 但其余一切（日志、模拟、截图）看起来完全正常，极容易被误判成"程序没显示"。 */
	{
		...
		log_info("  视频驱动=%s 渲染器=%s（%s）",
				 drv ? drv : "?",
				 have_ri ? ri.name : "?",
				 (have_ri && (ri.flags & SDL_RENDERER_ACCELERATED)) ? "硬件加速" : "软件");
		if (drv && (!strcmp(drv, "dummy") || !strcmp(drv, "offscreen")))
...
bool zm_display_event_loop(void (*on_click)(uint32_t x, uint32_t y), uint32_t timeout_ms) {
	if (!g_win) {
	...
	for (;;) {
		...
		while (SDL_PollEvent(&e)) {
			...
			if (e.type == SDL_MOUSEBUTTONDOWN && on_click) {
				uint32_t cx =
					(win_w > 0) ? (uint32_t)(e.button.x * g_w / win_w) : (uint32_t)e.button.x;
				uint32_t cy =
					(win_h > 0) ? (uint32_t)(e.button.y * g_h / win_h) : (uint32_t)e.button.y;
				on_click(cx, cy);
			...
			if (e.type == SDL_MOUSEMOTION && (e.motion.state & SDL_BUTTON_LMASK)) {
				uint32_t cx =
					(win_w > 0) ? (uint32_t)(e.motion.x * g_w / win_w) : (uint32_t)e.motion.x;
				uint32_t cy =
					(win_h > 0) ? (uint32_t)(e.motion.y * g_h / win_h) : (uint32_t)e.motion.y;
				on_touch_move(cx, cy);
...
uint32_t
zm_display_stub(uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	(void)uc;
	zm_display_slot_tick(off);
	log_debug("display stub[0x%X] r0=0x%X r1=0x%X r2=0x%X r3=0x%X", off, r0, r1, r2, r3);
	return 0;
}
...
uint32_t zm_display_CreateLayer(
...
uint32_t zm_display_CreateLayerExt(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
...
uint32_t zm_display_FreeLayer(uc_engine *uc, uint32_t r0, uint32_t r1) {
	...
	log_info("FreeLayer(层=%u) 缓冲@0x%X（自持标志=%u%s）→ 指针已清零",
			 r1,
			 buf,
			 own,
...
uint32_t zm_display_GetLayerInfo(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
...
uint32_t zm_display_SetLayerPosition(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	return zm_display_stub(uc, off, r0, r1, r2, r3);
}
 ...
 */
static uint32_t update_ex_impl(uc_engine *uc,
							   uint32_t display,
							   int32_t x,
							   int32_t y,
							   int32_t w,
...
uint32_t zm_display_Update(
	uc_engine *uc, uint32_t display, uint32_t x, uint32_t y, uint32_t w, uint32_t h) {
	zm_display_slot_tick(0x28U);
	return update_ex_impl(
		uc, display, (int32_t)x, (int32_t)y, (int32_t)w, (int32_t)h, ZM_UPDATE_DEFAULT_LIST, 4u);
}
...
uint32_t zm_display_RegisterCustomFont(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	return zm_display_stub(uc, off, r0, r1, r2, r3);
}
 ...
 */
uint32_t zm_display_SetActiveLayer(uc_engine *uc, uint32_t display, uint32_t idx) {
...
uint32_t zm_display_UpdateEx(
	uc_engine *uc, uint32_t display, uint32_t rect_ptr, uint32_t count, uint32_t list_ptr) {
	...
	uint32_t sig = ((uint32_t)x & 0xFFFu) ^ (((uint32_t)y & 0xFFFu) << 12) ^
				   (((uint32_t)w & 0xFFFu) << 8) ^ (((uint32_t)h & 0xFFFu) << 20) ^ (count << 4) ^
				   (lst[0] & 0xF) ^ ((lst[1] & 0xF) << 1) ^ ((lst[2] & 0xF) << 2) ^
				   ((lst[3] & 0xF) << 3);
	if (sig != sig_prev) {
		sig_prev = sig;
		sig_cnt++;
...
uint32_t zm_display_MeasureString(uc_engine *uc,
								  uint32_t disp,
								  uint32_t str_ptr,
								  uint32_t len,
								  uint32_t width_out,
	...
	if (sp) {
		...
		if (metrics_out) {
			TTF_Font *font = get_font(g_font_size);
			int fh = font ? TTF_FontHeight(font)
...
uint32_t zm_display_DrawText(
	uc_engine *uc, uint32_t rect_ptr, uint32_t text_ptr, uint32_t text_len, uint32_t sp) {
 ...
 * 擦不掉（拖影）。 */
static void layer_fill(uc_engine *uc, int x, int y, int w, int h, uint32_t color) {
	...
	if (depth == 1) {
		...
		uint16_t c16 = (uint16_t)(((color & 0xF80000u) >> 8) | ((color & 0xFC00u) >> 5) |
								  ((color & 0xFFu) >> 3));
		...
		for (int i = 0; i < npx; i++)
			row16[i] = c16;
		for (int yy = y0; yy < y1; yy++)
			uc_mem_write(uc, buf + (uint32_t)((yy * pitch + x0) * 2), row16, (size_t)npx * 2u);
	} else {
		...
		for (int i = 0; i < npx; i++)
			row32[i] = c32;
		for (int yy = y0; yy < y1; yy++)
			uc_mem_write(uc, buf + (uint32_t)((yy * pitch + x0) * 4), row32, (size_t)npx * 4u);
	}
}

uint32_t zm_display_FillRect(uc_engine *uc, uint32_t x, uint32_t y, uint32_t w, uint32_t sp) {
	...
	static uint32_t n = 0;
	if ((n++ % 200) == 0)
		log_info("IDisplay.FillRect(%d,%d,%d,%d, 0x%08X)（第 %u 次）",
				 (int)x,
				 (int)y,
				 (int)w,
				 h,
...
uint32_t zm_display_SetTransColor(uc_engine *uc, uint32_t r0, uint32_t r1, uint32_t r2) {
...
uint32_t zm_display_SetOpacity(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	return zm_display_stub(uc, off, r0, r1, r2, r3);
}
...
uint32_t zm_display_DrawLine(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	return zm_display_stub(uc, off, r0, r1, r2, r3);
}
uint32_t zm_display_DrawRoundRect(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	return zm_display_stub(uc, off, r0, r1, r2, r3);
}
uint32_t zm_display_DrawCircle(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	return zm_display_stub(uc, off, r0, r1, r2, r3);
}
uint32_t zm_display_FillCircle(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	return zm_display_stub(uc, off, r0, r1, r2, r3);
}
uint32_t zm_display_DrawArc(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	return zm_display_stub(uc, off, r0, r1, r2, r3);
}
uint32_t zm_display_FillArc(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	return zm_display_stub(uc, off, r0, r1, r2, r3);
}
uint32_t zm_display_FillGradientRect(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	return zm_display_stub(uc, off, r0, r1, r2, r3);
}
uint32_t zm_display_AlphaBlendRect(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	return zm_display_stub(uc, off, r0, r1, r2, r3);
}
...
uint32_t zm_display_DrawImage(
	...
	if (zm_image_get_pixels(r3, &w, &h, &rgba)) {
		...
		if (!known && nseen < 24) {
			seen[nseen++] = r3;
			log_info("[DrawImage 首次] obj=0x%X %dx%d 目标=(%d,%d) a5=%d",
					 r3,
					 w,
					 h,
					 (int)r1,
...
static int bitmap_info_to_rgba(uc_engine *uc,
							   uint32_t info,
							   int sl,
							   int st,
							   int sr,
	...
	if (sl < 0 || st < 0 || sr > bw || sb > bh || sr - sl <= 0 || sb - st <= 0) {
		 ...
		 * 刷屏，属预期，不是错误。 */
		log_debug(
			"IBitmap 子矩形 {%d,%d,%d,%d} 对 %dx%d 越界/为空，退回整图", sl, st, sr, sb, bw, bh);
		sl = 0;
		st = 0;
		sr = bw;
		sb = bh;
	...
	for (int y = 0; y < oh; y++) {
		...
		if (uc_mem_read(uc,
						...
						row,
						(size_t)ow * (size_t)bpp) != UC_ERR_OK) {
			free(row);
			free(rgba);
			return 0;
		}
		for (int x = 0; x < ow; x++) {
			...
			int is_key =
				enable_key && ((int32_t)trans >= 0) && ((uint32_t)(trans & mask) == (v & mask));
			o[3] = is_key ? 0 : a;
		}
	}
	free(row);
...
static int bitmap_obj_to_rgba(uc_engine *uc, uint32_t obj, int *w, int *h, uint8_t **out) {
	...
	if (!((obj >= BITMAP_POOL && obj < BITMAP_POOL + BITMAP_SLOT_COUNT * BITMAP_SLOT_SIZE) ||
		  obj == G_BITMAP_ADDR))
		return 0;
...
static int
blit_surface_region(uc_engine *uc, uint32_t obj, int dx, int dy, uint32_t rect_ptr, int mode) {
	int w = 0, h = 0;
	const uint8_t *rgba = NULL;
	...
	if (!zm_image_get_pixels(obj, &w, &h, &rgba)) {
		...
		uint32_t sub_surf = 0;
		if (uc_mem_read(uc, obj + 8, &sub_surf, 4) == UC_ERR_OK && sub_surf && sub_surf != obj &&
			zm_image_get_pixels(sub_surf, &w, &h, &rgba)) {
			obj = sub_surf;
	 ...
	 * 对上号（诊断"某张图没显示"时最直接：载入了但从未出现在这里 = 没被画）。 */
	{
		...
		if (idx < 0 && nseen < 24) {
			idx = nseen;
			seen[nseen++] = obj;
			log_info("[绘制首次] obj=0x%X %dx%d 目标=(%d,%d) mode=%d rect=%s",
					 obj,
					 w,
					 h,
					 dx,
	...
	for (int y = 0; y < sh; y++)
		memcpy(
			sub + (size_t)y * sw * 4u, rgba + ((size_t)(sy + y) * w + sx) * 4u, (size_t)sw * 4u);
...
uint32_t zm_display_DrawBitmap(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	(void)off;
	(void)r0;
	fb_refresh_draw_target();
...
uint32_t zm_display_DrawBitmapEx(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	(void)off;
	(void)r0;
	fb_refresh_draw_target();
...
uint32_t zm_display_DrawBitmapFrame(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	return zm_display_stub(uc, off, r0, r1, r2, r3);
}
uint32_t
zm_display_CreateBitmap(uc_engine *uc, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	...
	log_info("CreateBitmap(%dx%d fmt=%d) -> 0x%X pix@0x%X pal@0x%X",
			 w,
			 h,
			 fmt,
			 obj,
...
uint32_t zm_display_CreateImage(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	(void)off;
...
uint32_t zm_display_Flatten(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	return zm_display_stub(uc, off, r0, r1, r2, r3);
}
uint32_t zm_display_StretchBlt(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	...
	int sw = 0, sh = 0;
	uint8_t *rgba = NULL;
	if (!bitmap_info_to_rgba(
			uc, r2, sl, st, sl + sw_arg, st + sh_arg, mask_on != 0, &sw, &sh, &rgba))
		return 0;
	...
	if (++n <= 6)
		log_info("StretchBlt #%u 源图%dx%d 源区={%d,%d,%d,%d}→取到%dx%d "
				 "目标(%d,%d) %dx%d 色键=%d",
				 n,
				 (int)uc_read32(uc, r2 + 0),
				 (int)uc_read32(uc, r2 + 4),
				 sl,
				 st,
				 sw_arg,
				 sh_arg,
...
uint32_t zm_display_DrawAntialiasingLine(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	return zm_display_stub(uc, off, r0, r1, r2, r3);
}
uint32_t zm_display_DrawWLine(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	return zm_display_stub(uc, off, r0, r1, r2, r3);
}
uint32_t zm_display_GetDMLayerHdlr(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	(void)uc;
	(void)off;
	(void)r0;
...
uint32_t zm_display_RelevanceLayer(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	(void)uc;
	(void)off;
	(void)r0;
...
uint32_t zm_display_DrawImageExt(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	zm_display_slot_tick(off);
	...
	if (!getenv("ZM_NO_DRAWEXT_BLIT"))
		fb_blit_rgba_scaled_mode(dx, dy, dw, dh, rgba, sw, sh, flags & 7, cx, cy, cw, ch);
	static uint32_t n = 0;
	if (++n <= 6)
		log_info("DrawImageExt #%u: 图像%dx%d → (%d,%d) %dx%d flags=%d",
				 n,
				 sw,
				 sh,
				 dx,
...
uint32_t zm_display_DrawSysWallPaper(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	return zm_display_stub(uc, off, r0, r1, r2, r3);
}
uint32_t zm_display_DrawBorderText(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	return zm_display_stub(uc, off, r0, r1, r2, r3);
}
uint32_t zm_display_PushAndSetAlphaLayer(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	return zm_display_stub(uc, off, r0, r1, r2, r3);
}
uint32_t zm_display_PopAndRestoreAlphaLayer(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	return zm_display_stub(uc, off, r0, r1, r2, r3);
}
uint32_t zm_display_RotateScreen(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	return zm_display_stub(uc, off, r0, r1, r2, r3);
}
...
uint32_t zm_bitmap_SetTransColor(uc_engine *uc, uint32_t r0, uint32_t r1) {
		...
		uc_write32(uc, r0 + 20, r1);
	{
		static uint32_t n = 0;
		if (n++ < 8)
			log_info("IBitmap.SetTransColor(bitmap=0x%X, color=0x%08X)（第 %u 次）", r0, r1, n);
	}
	return r0;
}
