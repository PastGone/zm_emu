...
uint32_t
zm_display_Update(uc_engine *uc, uint32_t display, uint32_t x, uint32_t y, uint32_t w, uint32_t h);
...
uint32_t zm_display_SelectFont(uc_engine *uc,
							   uint32_t display,
							   uint32_t font_idx); /* +0x40，真机虚表 SelectFont(this, idx) */
uint32_t
zm_display_GetFontHeight(uc_engine *uc); /* +0x48，真机虚表 GetFontHeight（实现待 RE 校准） */
uint32_t zm_display_MeasureString(
	uc_engine *uc,
	uint32_t disp,
	uint32_t str_ptr,
	uint32_t len,
...
uint32_t zm_display_DrawText(uc_engine *uc,
							 uint32_t rect_ptr,
							 uint32_t text_ptr,
							 uint32_t text_len,
							 uint32_t sp); /* +0x50 */
uint32_t
zm_display_DrawRect(uc_engine *uc, uint32_t x, uint32_t y, uint32_t w, uint32_t sp); /* +0x6C */
uint32_t
zm_display_FillRect(uc_engine *uc, uint32_t x, uint32_t y, uint32_t w, uint32_t sp); /* +0x70 */
...
uint32_t zm_display_CreateImage(
	uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3);
uint32_t
zm_display_BitBlt(uc_engine *uc, uint32_t off, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3);
