...
void zm_image_dump_pool(void) {
	...
	for (int i = 0; i < IMAGE_SLOT_COUNT; i++) {
		if (!g_img[i].used)
			continue;
		log_info("     槽%-3d %-5s %4dx%-4d ref=%d obj=0x%X \"%s\"",
				 i,
				 g_img[i].kind == REC_ENTRY ? "entry" : (g_img[i].kind == REC_SURF ? "surf" : "?"),
				 g_img[i].w,
				 g_img[i].h,
				 g_img[i].refcnt,
				 img_addr(i),
	...
	for (int i = 0; i < BITMAP_SLOT_COUNT; i++) {
		...
		log_info("     槽%-3d %-5s %4dx%-4d ref=%d obj=0x%X \"%s\"",
				 i,
				 g_bmp[i].kind == REC_ENTRY ? "entry" : (g_bmp[i].kind == REC_SURF ? "surf" : "?"),
				 g_bmp[i].w,
				 g_bmp[i].h,
				 g_bmp[i].refcnt,
				 bmp_addr(i),
...
static int decode_via_sdl(const uint8_t *data, size_t len, int *ow, int *oh, uint8_t **orgba) {
	zm_img_ready();
...
static int decode_png(const uint8_t *data, size_t len, int *ow, int *oh, uint8_t **orgba) {
...
static int decode_jpg(const uint8_t *data, size_t len, int *ow, int *oh, uint8_t **orgba) {
...
static int decode_any(const uint8_t *data, size_t len, int *w, int *h, uint8_t **rgba, int *type) {
...
static int rec_load_file(zm_img_rec *r, const char *name) {
	...
	log_info("IImage: 载入 \"%s\" %dx%d type=%d(%s)",
			 name,
			 w,
			 h,
			 type,
			 type == ZM_IMG_TYPE_PNG ? "PNG" : (type == ZM_IMG_TYPE_JPG ? "JPG" : "GIF"));
	return 0;
}
...
static int rec_load_mem(uc_engine *uc, zm_img_rec *r, uint32_t ptr, uint32_t len) {
	...
	log_info("IImage: 载入内存图像 0x%X %u 字节 -> %dx%d type=%d(%s)",
			 ptr,
			 len,
			 w,
			 h,
...
uint32_t zm_image_CreateImage(uc_engine *uc, uint32_t r0, uint32_t r1, uint32_t r2, uint32_t r3) {
	...
	log_debug("IDisplay::CreateImage -> entry@0x%X surf@0x%X (slot %d)", obj, surf, idx);
	return 0; /* 0 = 成功 */
}
...
uint32_t zm_image_GetType(uc_engine *uc, uint32_t r0) {
	 ...
	 * 对象是哪张图？—— 决定要不要把魔数映射成真机的类型枚举。 */
	{
		...
		if (n < 8) {
			...
			zm_img_rec *r = pixel_rec(r0);
			n++;
			log_info("[IImage.GetType] obj=0x%X → %u (%s) 调用点 0x%X",
					 r0,
					 t,
					 r && r->name[0] ? r->name : "池外对象",
					 lr);
		}
	}
	return t;
...
uint32_t zm_image_DecodeToBitmap(
		...
		if (gpx && src->rgba) {
			...
			for (int yy = 0; yy < src->h; yy++) {
				...
				while (done < src->w) {
					...
					uc_mem_write(uc,
								 gpx + (uint32_t)yy * (uint32_t)src->w * 4u + (uint32_t)done * 4u,
								 line,
								 (size_t)n * 4u);
					done += n;
				}
	...
	log_debug("IImage::Decode -> surf=0x%X %dx%d (out@0x%X)", surf_obj, src->w, src->h, out_ptr);
	return 0;
}
...
int zm_image_get_gdi_surface(
	uc_engine *uc, uint32_t surf, int *w, int *h, int *step_out, uint32_t *colorkey) {
	uint32_t v[4];
...
static int gdi_read_palette(uc_engine *uc, uint32_t pal_obj, int *trans_idx, uint32_t *pal) {
	uint32_t vi = 0, vp = 0;
...
static int gdi_read_pixel(uc_engine *uc, uint32_t addr, int fmt, uint32_t ck, int *out_argb) {
	if (fmt == 1) { /* RGB565 */
		uint16_t v = 0;
...
int zm_image_blit_gdi_surface(
	...
	int ncand = 0;
	uint32_t dp = 0;
	if (uc_mem_read(uc, surf + GDI_SURFACE_HDR, &dp, 4) == UC_ERR_OK && dp >= 0x1000u &&
		dp < 0xFFF00000u)
		base_cand[ncand++] = dp;
	base_cand[ncand++] = surf + GDI_SURFACE_HDR;
	if (uc_mem_read(uc, surf + 8, &dp, 4) == UC_ERR_OK && dp >= 0x1000u && dp < 0xFFF00000u)
		base_cand[ncand++] = dp;
	...
	for (int y = 0; y < sh; y++) {
		for (int x = 0; x < sw; x++) {
			int argb = 0;
			int rc = gdi_read_pixel(
				uc, base + ((size_t)(sy + y) * w + (sx + x)) * bytes, step, ck, &argb);
			if (rc <= 0)
				continue; /* 透明/越界 */
			int ddx = x, ddy = y;
