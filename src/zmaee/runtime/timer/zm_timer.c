...
uint32_t zm_timer_StartTimer(uc_engine *uc, uint32_t interval_ms, uint32_t id, uint32_t cb) {
	...
	log_info(
		"ZMAEE_Start_Timer(每 %ums, id=0x%08X, cb=0x%08X) -> 槽 %d", interval_ms, id, cb, slot);
...
static bool zm_timer_poll_repeat(uint32_t now_ms) {
	for (uint32_t i = 0; i < ZM_RPT_MAX; ++i) {
		...
		log_info("周期定时器(槽 %u, 每 %ums) 到期 -> cb=0x%08X", i, s_rpt[i].interval, cb);
		return true;
	}
	return false;
}
...
bool zm_timer_async_call(uc_engine *uc,
	 ...
	 * 排查 0000042f 在"返回主菜单"时崩（PC=0x3A48 实测是合法 ARM `str fp,[sp]`）时加的。 */
	{
		...
		if (n < 3 && nargs <= 2) {
			...
			log_info("%s 异步派发 -> cb=0x%08X(arg0=0x%X)（打断 PC=0x%X）",
					 what,
					 cb,
					 nargs > 0 ? args[0] : 0,
					 resume_pc);
